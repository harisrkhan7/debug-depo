#!/usr/bin/env python3
"""Apply and verify the local mini-swe-agent-plus integration patches."""
...
def patch_litellm_model(path: Path) -> None:
    ...
    if hash_import not in text:
        ...
        if import_marker not in text:
            raise SystemExit(f"Could not patch local vLLM hash import in {path}")
        text = text.replace(import_marker, import_marker + hash_import, 1)
...
def patch_add_edit_config(path: Path) -> None:
    text = path.read_text()
...
def patch_singularity_environment(path: Path) -> None:
    original_text = path.read_text()
    text = original_text
    ...
    pid_exec = '        cmd.extend(["--contain", "--cleanenv", "--pid"])\n'
    upstream_exec = '        cmd.extend(["--contain", "--cleanenv"])\n'
    if pid_exec in text:
        text = text.replace(pid_exec, upstream_exec, 1)
    elif upstream_exec not in text:
        raise SystemExit(f"Could not identify the Apptainer exec options in {path}")
...
def patch_checkout(checkout: Path) -> None:
    patches = (
        (
            checkout / "src/minisweagent/models/litellm_model.py",
            patch_litellm_model,
        ),
        (
            checkout / "src/minisweagent/config/extra/swebench_add_edit_tool.yaml",
            patch_add_edit_config,
        ),
        (
            checkout / "src/minisweagent/environments/singularity.py",
            patch_singularity_environment,
        ),
    )
...
def main() -> None:
    ...
    patch_parser = subparsers.add_parser("patch", help="patch an external checkout")
    patch_parser.add_argument("checkout", type=Path)
    subparsers.add_parser("verify", help="verify the installed integration")
    args = parser.parse_args()

    if args.action == "patch":
        patch_checkout(args.checkout)
    else:
        verify_install()
